...
// ===== 代理服务端配置 =====
// 壳插件不直接请求B站，而是转发给本地Termux代理服务端
// 代理服务端用Node.js fetch无限制
// 用户需在插件设置填 proxyUrl，如 http://localhost:3000
function getProxyUrl() {
    try {
        const v = env.getUserVariables();
        return (v && v.proxyUrl && String(v.proxyUrl).trim()) || "";
    } catch (e) {
        return "";
    }
}
// 通过代理请求B站接口
// proxyGet(bilibiliUrl, { cookie, referer }) → 返回B站响应JSON
async function proxyGet(bilibiliUrl, options) {
    const proxyBase = getProxyUrl();
    if (!proxyBase) {
        throw new Error("未配置代理服务端地址(proxyUrl)，请在插件设置填写。参考：Termux运行server.js后填 http://localhost:3000");
    }
    const opts = options || {};
    const params = new URLSearchParams();
    params.set('url', bilibiliUrl);
    if (opts.cookie) params.set('cookie', opts.cookie);
    if (opts.referer) params.set('referer', opts.referer);
    const proxyFullUrl = proxyBase.replace(/\/$/, '') + '/proxy?' + params.toString();
    const resp = await fetch(proxyFullUrl, { headers: { 'Accept': 'application/json' } });
    if (!resp.ok) {
        throw new Error("代理请求失败 " + resp.status + " " + resp.statusText);
    }
    return await resp.json();
}
// axios 兼容层：让原版 axios.get(url, {params, headers}) 代码不用改
// 自动从 params 拼 URL，从 headers 提取 cookie/referer，调用 proxyGet
// 返回 {data: 响应体} 伪装成 axios 返回值
async function axiosCompatGet(url, config) {
    const cfg = config || {};
    let fullUrl = url;
    if (cfg.params) {
    ...
    const h = cfg.headers || {};
    const data = await proxyGet(fullUrl, {
        cookie: h.cookie || h.Cookie || '',
        referer: h.referer || h.Referer || 'https://www.bilibili.com/',
    });
    return { data: data };
}
async function axiosCompatPost(url, body, config) {
    // POST 也走代理（用 GET 模拟，因为代理服务端只实现了 GET）
    // 大部分B站POST接口（如 GenWebTicket）也可以用 GET 参数
    const cfg = config || {};
    ...
    const data = await proxyGet(fullUrl, {
        cookie: h.cookie || '',
        referer: h.referer || 'https://www.bilibili.com/',
    });
    return { data: data };
}
...
async function getCookie() {
    if (!cookie) {
        cookie = (await axiosCompatGet("https://api.bilibili.com/x/frontend/finger/spi", {
...
async function getCid(bvid, aid) {
    ...
    const cidRes = (await axiosCompatGet("https://api.bilibili.com/x/web-interface/view", {
...
async function searchBase(keyword, page, searchType) {
    ...
    try {
        const res = (await axiosCompatGet("https://api.bilibili.com/x/web-interface/search/type", {
...
async function getFavoriteList(id) {
    ...
    while (true) {
        try {
            const { data: { data: { medias, has_more }, }, } = await axiosCompatGet("https://api.bilibili.com/x/v3/fav/resource/list", {
                params: {
                    media_id: id,
                    platform: "web",
                    ps: pageSize,
...
async function getBiliTicket(csrf) {
    ...
    try {
        const response = await axiosCompatPost(url, null, {
            params: {
                key_id: 'ec02',
                hexsign: hexSign,
                'context[ts]': ts,
...
async function getWWebId(id) {
    ...
    const html = (await axiosCompatGet("https://space.bilibili.com/" + id, {
        headers: {
            "user-agent": getUserUA(),
        }
    })).data;
...
async function getArtistWorks(artistItem, page, type) {
    ...
    try {
        const res = (await axiosCompatGet("https://api.bilibili.com/x/space/wbi/arc/search", {
...
async function getMediaSource(musicItem, quality) {
    ...
    try {
        ...
        const res = (await axiosCompatGet("https://api.bilibili.com/x/player/playurl", {
            headers: headers,
...
async function getTopLists() {
    ...
    const weeklyRes = await axiosCompatGet("https://api.bilibili.com/x/web-interface/popular/series/list", {
        headers: Object.assign(Object.assign({}, headers), { cookie: await getCookieString() }),
    });
...
async function getTopListDetail(topListItem) {
    ...
    await getCookie();
    try {
        const res = await axiosCompatGet(`https://api.bilibili.com/x/web-interface/${topListItem.id}`, {
...
async function getMusicComments(musicItem, page) {
    ...
    try {
        const res = (await (axiosCompatGet("https://api.bilibili.com/x/v2/reply/main", {
            params: params,
...
async function getLyric(musicItem) {
    ...
    try {
        ...
        const playerRes = (await axiosCompatGet("https://api.bilibili.com/x/player/wbi/v2", {
            params: playerParams,
        ...
        async function fetchSubtitleBody(sub) {
            ...
            const r = (await axiosCompatGet(u, { headers: { "User-Agent": getUserUA(), referer } })).data;
...
module.exports = {
    ...
    userVariables: [
        ...
        },
        {
            key: "proxyUrl",
            name: "代理服务端地址",
            hint: "必填。Termux代理服务端地址。先在Termux里运行server.js（见GitHub仓库bili-proxy/server.js），然后填 http://localhost:3000 。填了之后所有B站请求走代理，绕开MusicFree沙箱网络限制。不填则插件无法工作。"
        }
    ],
    hints: {
        importMusicSheet: [
