...
// ===== fetch 版网络请求封装 =====
// 用全局 fetch 替代 axios，避免 axios 自动加 Accept/Accept-Encoding 暴露脚本身份
// fetch 行为更接近浏览器，MusicFree(RN) 环境下 fetch 更原生
async function fetchGet(url, options) {
    ...
    const resp = await fetch(fullUrl, { headers: opts.headers || {} });
    if (!resp.ok) {
        throw new Error("fetch " + resp.status + " " + resp.statusText + " url=" + fullUrl.slice(0, 100));
    }
    return await resp.json();
}
async function fetchPost(url, options) {
    const opts = options || {};
    const resp = await fetch(url, { method: 'POST', headers: opts.headers || {}, body: opts.body });
    if (!resp.ok) {
        throw new Error("fetch POST " + resp.status + " " + resp.statusText);
    }
    return await resp.json();
}
...
async function getCookie() {
    if (!cookie) {
        cookie = (await fetchGet("https://api.bilibili.com/x/frontend/finger/spi", {
...
async function getCid(bvid, aid) {
    ...
    const cidRes = (await fetchGet("https://api.bilibili.com/x/web-interface/view", {
...
async function searchBase(keyword, page, searchType) {
    ...
    try {
        const res = (await fetchGet("https://api.bilibili.com/x/web-interface/search/type", {
...
async function getFavoriteList(id) {
    ...
    while (true) {
        try {
            const { data: { medias, has_more } } = await fetchGet("https://api.bilibili.com/x/v3/fav/resource/list", {
                params: {
                    media_id: id,
                    platform: "web",
                    ps: pageSize,
...
async function getBiliTicket(csrf) {
    ...
    try {
        const response = await fetchPost(url, null, {
            params: {
                key_id: 'ec02',
                hexsign: hexSign,
                'context[ts]': ts,
...
async function getWWebId(id) {
    ...
    const html = (await fetchGet("https://space.bilibili.com/" + id, {
        headers: {
            "user-agent": getUserUA(),
        }
    }));
...
async function getArtistWorks(artistItem, page, type) {
    ...
    try {
        const res = (await fetchGet("https://api.bilibili.com/x/space/wbi/arc/search", {
...
async function getMediaSource(musicItem, quality) {
    ...
    try {
        ...
        const res = (await fetchGet("https://api.bilibili.com/x/player/playurl", {
            headers: headers,
...
async function getTopLists() {
    ...
    const weeklyRes = await fetchGet("https://api.bilibili.com/x/web-interface/popular/series/list", {
        headers: Object.assign(Object.assign({}, headers), { cookie: await getCookieString() }),
    });
    weekly.data = weeklyRes.data.list.slice(0, 8).map((e) => ({
...
async function getTopListDetail(topListItem) {
    ...
    await getCookie();
    try {
        const res = await fetchGet(`https://api.bilibili.com/x/web-interface/${topListItem.id}`, {
        ...
        const list = ((_a = res.data) === null || _a === void 0 ? void 0 : _a.list) || [];
...
async function getMusicComments(musicItem, page) {
    ...
    try {
        const res = (await (fetchGet("https://api.bilibili.com/x/v2/reply/main", {
            params: params,
...
async function getLyric(musicItem) {
    ...
    try {
        ...
        const playerRes = (await fetchGet("https://api.bilibili.com/x/player/wbi/v2", {
            params: playerParams,
        ...
        async function fetchSubtitleBody(sub) {
            ...
            const r = (await fetchGet(u, { headers: { "User-Agent": getUserUA(), referer } }));
